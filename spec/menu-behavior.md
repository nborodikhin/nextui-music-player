...
### Send Crash Report (conditional, inserted above Settings)

Independently of the State A/B/C variants, a `Send Crash Report` row is inserted immediately above `Settings` when an actionable crash report exists. The row never displaces `Resume` or `Now Playing` — those keep their position at index 0 — so muscle memory for "open app → A → resume playback" is preserved.

**Condition** (evaluated on every frame): `Settings_getCollectCrashReports()` returns `true` AND the newest subdirectory under `<SHARED_USERDATA_PATH>/music-player/crash-reports/` does *not* contain a `skipped.txt`. Older non-skipped tickets do not resurface — the model is "is the latest crash unskipped?".

**Effect on item count and indices**: when present, the item count increases by 1 and `Settings` shifts down by 1. Example with State C + crash:
...
[2] Online Radio
[3] Podcasts
[4] Send Crash Report           ← new
[5] Settings
```

Selecting the row opens the Crash Report dialog (see Overlay Dialogs below). See `spec/crash-reporting.md` for the full feature spec.
...
| Armed, `now - exit_armed_at < TOAST_DURATION` | B | Replaces the toast with `"Exiting..."`, renders it, then blocks for `EXIT_TOAST_DELAY_MS` (100 ms) before returning `MENU_QUIT` (-1). |
...
The pause on the confirming press exists because NextUI itself takes roughly half a second to render its own UI after the process exits; without it, the screen would show nothing between the confirming B-press and NextUI's UI appearing, leaving the user without feedback that their press was registered. The `"Exiting..."` toast is rendered and flipped to screen before the delay, so it is guaranteed to be visible during that window.
...
Triggered by: A on the conditional `Send Crash Report` row.

Title: `"Crash Report"`

Body shows the SD-relative path to the newest unsent bundle and instructions to share it (GitHub issue or Discord).

| Button | Outcome |
|--------|---------|
| A on `Close` | Dismisses dialog, no state change. Row remains. |
| A on `Skip this report` | Writes `skipped.txt` in the ticket's directory. Row disappears. Next crash brings the row back. |
| A on `Never collect crash data` | Calls `Settings_setCollectCrashReports(false)`. Row disappears. Signal handler short-circuits on future crashes. Reversible via Settings. |
| B | Acts as `Close` |

See `spec/crash-reporting.md` for the full state machine and signal-handler discipline.

---

### TODO: dialogs should respect power management while open

The Main Menu loop itself calls `ModuleCommon_PWR_update()` every frame, so NextUI's device-wide auto-suspend (deep sleep) works as expected when the menu is idle. **But** all overlay dialogs in the app (Controls Help, Quit Confirmation, Crash Report, the confirmation dialogs in Playlist Manager / File Browser, Add-to-Playlist, the keyboard) `continue` past `ModuleCommon_handleGlobalInput()` and `ModuleCommon_PWR_update()` on the frames they're open. Consequences only while a dialog is actually showing:

- **Power button → deep sleep** is ignored.
- **Hardware volume / brightness / color-temp** overlays don't appear.
- Any other behavior driven by the global input handler is suppressed.

**Model to follow**: the Settings module's clear-cache confirmation dialog (`module_settings.c`, `SETTINGS_STATE_CLEAR_CACHE_CONFIRM`) gets this right today — it renders the dialog inside the state machine but falls through to `ModuleCommon_PWR_update(&dirty, &show_setting)` at the bottom of the loop, so power and hardware overlays still work. The seven affected dialogs (Quit Confirmation, Controls Help, START press-and-hold wait, Crash Report, Playlist Manager confirm, File Browser delete confirm, Add-to-Playlist) should be restructured to match: handle dialog-specific input, render dialog overlay, but do **not** `continue` past the loop's PWR_update / handleGlobalInput call. Touching this affects every dialog in the app — schedule as a focused refactor rather than per-feature.
