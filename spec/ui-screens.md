...
- Item list (4 or 5 items depending on state):
...
B button → two-step exit. First press shows toast `"Press B again to exit"`. Second B-press within 3 s exits the app. Other buttons during the window behave normally and do not cancel the arm. After 3 s the toast clears and the next B press is treated as a fresh first press. See `spec/menu-behavior.md` for the full state machine.
...
What the user sees:
- Title bar: `"Settings"`
- 6 items with current values shown inline:
...
There is no screen below the Main Menu — popping it means exiting the app. To prevent accidental exits, the Main Menu uses a two-step B gesture: the first press shows a 3-second toast `"Press B again to exit"`, and a second B-press while that toast is visible exits the app. Other buttons during the window behave normally and do not cancel the arm. See `spec/menu-behavior.md` for details.
