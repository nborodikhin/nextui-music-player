...
//
// THREAD SCOPE: the dog watches the main loop and nothing else. heartbeat/pause/
// resume are no-ops when called from any other thread, so a worker can neither
// forge main-loop liveness nor clear a pause reason the main thread is holding.
// Worker code may therefore call these unconditionally — Wifi_ensureConnected()
// does, from both main and the podcast download thread — and get the right
// behavior on each. Blocking on a worker is not a hang and needs no pause.
...
// Bind the calling thread as the watched main loop. Called by Watchdog_init();
// exposed separately so host tests can establish the binding without spawning a
// thread. Until it is called nothing is bound and every caller counts as main.
...
// Must be called from the main loop's thread — it binds that thread as watched.
...
// No-op off the main thread (see THREAD SCOPE above).
...
// no reasons are set. Idempotent per-reason. No-ops off the main thread.
//
// Because the mask is a bit per reason rather than a depth counter, nesting two
// pauses of the SAME reason and resuming the inner one clears the outer's hold.
// No current call path nests a reason; keep it that way, or switch to counters.
//
